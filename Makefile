...
# Run the opt-in S3 smoke test against a real, remote PanUKBB-style HailTable + BlockMatrix
# (issue #22). Requires httpfs and network access; skips cleanly (not a failure) if the path env
# vars below aren't set, so plain `make test` never depends on network access. No AWS credentials
# are needed for the public PanUKBB bucket.
# Usage:
